...
*/
priority_queue<Pixel,vector<Pixel>,cmp1>initprioq;
//#pragma omp parallel for reduction(+:tempcounter,id)
for(int y=0;y<rows;y++){
      ...
      for(int z=0;z<cols;z++){
           ...
           if(markers(y,z)==2){    
            markerMap(y,z) = id;
            temp(tempcounter,0)=image(y,z);
	    temp(tempcounter,1)=y;
            temp(tempcounter,2)=z;
            tempcounter++;
            id++;
            initprioq.push({temp(tempcounter,0),temp(tempcounter,1),temp(tempcounter,2)});	    
           }
      }
}
...
for(int i=0;i<tempcounter;i++){
  temp(tempcounter,0)=initprioq.top().val;
  temp(tempcounter,1)=initprioq.top().x;
  temp(tempcounter,2)=initprioq.top().y;
  initprioq.pop();
  ...
  while(!prioq.empty()){
    ...
    for(int i = 0; i < 4; i++) {
       ...
       if(nextX < 0 || nextY < 0 || nextX >= rows || nextY >= cols) {
        ...
        if(markerMap(nextX,nextY) != -1 && image(nextX,nextY)!=0&&image(CrtX,CrtY)!=0){
                    if(markerMap(CrtX,CrtY) == -1) {
                        
			markerMap(CrtX,CrtY) = markerMap(nextX,nextY);//using id value,all strats from their closest neighbour
	 ...
	 else if(markerMap(nextX,nextY) == -1&&image(nextX,nextY)!=0&&image(CrtX,CrtY)!=0){
			...
			if(markerMap(CrtX,CrtY)!=-1&&inprioq(CrtX,CrtY)==false){
                           inprioq(CrtX,CrtY)==true;
                           duplImage.at<Vec3b>(CrtX,CrtY)=colors[ markerMap(CrtX,CrtY) ];
